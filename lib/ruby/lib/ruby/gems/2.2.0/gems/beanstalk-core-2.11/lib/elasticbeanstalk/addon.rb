...
module ElasticBeanstalk
    ...
    class AddonManager
        ...
        def search_enabled_addons
            addon_names = Dir.glob("#{@addon_path}/*/#{ADDON_CONF_FILENAME}").collect do |path|
                name = Pathname(path).each_filename.to_a[-2]
                if name == 'sqsd'
                    ...
                    name = '' if ! commands.has_key?('CMD-SqsdDeploy')
                end
                name
            end

            addon_names.reject! { |name| name.length < 1 }
