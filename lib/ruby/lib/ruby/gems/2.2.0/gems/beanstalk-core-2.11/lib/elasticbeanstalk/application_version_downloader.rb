...
module ElasticBeanstalk
    class ApplicationVersionDownloader
        ...
        def download_from_s3_location (destination:, s3_bucket:, s3_key:, s3_version: nil)
            ...
            begin
                if s3_version
                    s3_obj = s3.bucket(s3_bucket).object(s3_key).version(s3_version)
                    head = s3_obj.head
                else
                ...
                @logger.info "Size: #{head.content_length}, ETag: #{head.etag}, Metadata: #{head.metadata}."
                s3_obj.get(response_target: destination)
                actual_size = File.size(destination)
                @logger.info "Downloaded size: #{actual_size}."
                if (actual_size != head.content_length)
                    raise RetriableDownloadError.new("Size mismatch: expected: #{head.content_length}, actual: #{actual_size}.")
                end
