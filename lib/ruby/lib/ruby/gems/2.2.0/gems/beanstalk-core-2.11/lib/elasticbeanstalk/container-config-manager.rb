...
module ElasticBeanstalk
    class ContainerConfigManager
        DEFAULT_CONFIG_FILE = Constants.containerconfig_file

        def self.refresh(env_metadata:  nil, logger: Logger.new(File.open(File::NULL, "w")))
            env_metadata ||= EnvironmentMetadata.new

            FileUtils.rm_rf(Constants.deploy_config_dir)
            FileUtils.mkdir_p(Constants.deploy_config_dir)
            logger.info("Recreated directory #{Constants.deploy_config_dir}.")

            appsource_url = env_metadata.app_source_url
            File.write(Constants.appsourceurl_file, JSON.pretty_generate({'url' => appsource_url}))
            logger.info("Created appsource url file at #{Constants.appsourceurl_file}.")

            container_config = env_metadata.container_config
            File.write(Constants.containerconfig_file, container_config.to_json)
            Utils.set_eb_datafile_permission(Constants.containerconfig_file)
            logger.info("Created container config file at #{Constants.containerconfig_file}.")
        end
        ...
        def system_config(key)
            system_configs = @container_configs['system']
