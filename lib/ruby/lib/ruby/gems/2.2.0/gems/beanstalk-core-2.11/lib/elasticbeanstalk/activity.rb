...
module ElasticBeanstalk
    ...
    class Activity
        ...
        ACTIVITY_TIMEOUT_START_LOG_MSG = 'Starting activity with a timeout of %{timeout_sec} seconds...'
        ...
        def timeout_exec(&block)
            begin
                ...
                if @timeout.nil? || @timeout == 0
                    ...
                    @logger.info(activity_path){ACTIVITY_TIMEOUT_START_LOG_MSG % {timeout_sec: @timeout}}
                    timeout(@timeout) do
                        Thread.new(activity_stack.clone) do |parent_activity_stack|
                            Thread.current[:activity_stack] = parent_activity_stack
                            result = exec(&block)
                        end.value
                    end
                end
