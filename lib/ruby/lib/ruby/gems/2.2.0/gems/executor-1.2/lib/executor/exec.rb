...
module Executor
    ...
    class Exec
        ...
        def exec(command, env:, block: nil)
            ...
            time_start = Time.now
            begin
                # launch subprocess with new process group
                Open3.popen2e(env, command, pgroup: true) do |sin, sout, wait_thr|
                    o.pid = wait_thr.pid
                    begin
                        lines = sout.collect do |line|
                    ...
                    o.exitstatus = wait_thr.value.exitstatus
                end
            ensure
                begin
                    # send SIGTERM to the subprocess group, if main process is interrupted
                    # if subprocess already exits by itself we just ignore ESRCH and continue
                    Process.kill('TERM', -Process.getpgid(o.pid)) if o.pid
                rescue Errno::ESRCH
                end
            end
            time_end = Time.now
